...
def get_last_video_file_path():
    files = [
        x for x in glob(f"{base_directory}/**") if is_video_file(os.path.abspath(x))
    ]
    return os.path.abspath(max(files, key=os.path.getctime))


def get_default_destination_file_path(from_file_path):
    destination_directory_path = os.path.abspath(
        os.path.join(from_file_path, os.pardir)
    )
